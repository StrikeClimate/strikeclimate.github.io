            ...
            {CoreValue?.map((item, index) => (
              ...
              >
                <Image
                  ...
                  className="mb-4"
                />
                <h3 className="text-center text-lg font-semibold mb-2 w-64 text-secondary leading-tight">{item.text}</h3>
                <p className="text-sm font-light text-center text-secondary">{item.description}</p>
              </motion.div>
            ))}
          </motion.div>
        </AnimatePresence>
